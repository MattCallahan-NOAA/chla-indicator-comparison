...
**Load occ data from previous couple hundred lines**
```{r}
...
ind%>%filter(INDICATOR_NAME=="Spring_Chlorophylla_Biomass_SMBKC_Satellite")%>%
  dplyr::select(YEAR, DATA_VALUE)%>%
  left_join(mod_smbkc, by="YEAR")

#OCC
occ_smbkc<-occ_esp%>%
  filter(BS_KING=="StMattBKC" & month%in%c(4:6))%>%
   group_by(year) %>%
  summarise(OCC_VALUE=mean(chlorophyll,na.rm=TRUE))

smbkc_chla<-occ_smbkc%>% left_join(mod_smbkc%>%dplyr::select(!crab), by=c("year"="YEAR"))
smbkc_chla%>%
  ggplot()+
  geom_line(aes(x=year,y=OCC_VALUE), color="red")+
